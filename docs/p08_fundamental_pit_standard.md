...
正式速率默认 400 次/分钟。供应商适配器拥有唯一请求限流器，因此重试和未来分页调用
都计入同一容量，而不是只对外层任务计数。
...
- 已完成 2014 年起的全市场财务 warm-up 与历史回补；
- 已接入具有生效区间的历史申万一级行业成员；
- 已冻结并实现首个基本面因子 `sp_ttm`，其工程契约见
  `docs/factor_research_standard.md`；
- SP 的真实数据预检属于链路验证，不构成投资结论；正式 P0.7/P0.6.3
  研究产物必须在 clean Git 版本上重新生成并分别通过门禁。
