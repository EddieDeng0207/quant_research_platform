...
| `daily_bars` | 未复权 OHLCV/金额 | 仅有可复算停牌状态或已审阅非交易区间时允许缺失 |
...
北交所 A 股研究域从交易所正式开市日 `2021-11-15` 起算。供应商把此前
新三板精选层行情回溯到 `.BJ` 命名空间的记录不属于本层研究域；有效上市日
取供应商上市日与北交所开市日的较晚者。排除的历史股票池行和行情行分别计数。

## 状态机与默认拒绝

1. `S` 且无行情使证券进入停牌状态；状态跨交易日和年度延续，直到出现真实行情，或出现不与 `S` 冲突的 `R`。
2. 同日同时出现 `S/R` 且无行情时，`S` 优先，不能仅凭 `R` 猜测已经恢复；真实行情优先级最高并结束跨日停牌状态。
3. 每行保存 `vendor_is_suspended`、`carried_forward_suspension` 和 `suspension_state_source`。同日事件、跨日延续和人工审阅区间不可混写。
4. 历史股票池有证券、日线缺失、且既无状态机证据也无已审阅区间：认定无法解释的数据缺口；禁止买卖并使产物晋级失败。
5. 有日线但无涨跌停记录：禁止买卖并使产物晋级失败。
6. 日内最低价低于官方跌停价或最高价高于官方涨停价（容差 0.0051 元）：使产物晋级失败。
7. `S` 与日线同时存在：标记部分/盘中停牌，按保守策略禁止当日交易；行情仍可用于收盘估值。
...
- 供应商漏报或错误返回普通边界时，只有
  `configs/p05_reviewed_market_events.json` 中精确匹配证券与日期、并绑定
  HTTPS 证据的 IPO/恢复上市/重新上市/退市整理首日，才可改判为
  `none_reviewed_market_event`；禁止按价格越界结果反推制度。
...
人工别名还必须分别声明法律主体连续性与业务连续性。交易和收益率层只在
`legal_continuity=true` 时连接价格链；基本面层遇到 `business_continuity=false`
必须在生效日截断，不得因法律证券延续而假设重组前后业务可比。
...
年度连续发布时可用上一年度已晋级 P0.5 产物作为停牌状态种子。构建器先
核对上一产物的 promotion、Parquet SHA-256 和结束日期，再提取每个证券最后
状态；上一 artifact ID、物理/逻辑哈希和种子证券数写入下一年度 manifest。
不提供上一产物时，构建器回读 `2016-01-01` 起的精简行情键和停复牌事件，
两条路径使用同一个状态机。
...
.venv/bin/qrp-data build-tradability \
  ...
  --prior-tradability-artifact data/curated/tradability/artifact_id=<2024_ID>
```
...
早期试点记录在 `artifacts/reports/p05_tradability_20240102_20240105.md`；
2016–2026 全市场修复、年度链与最终审计结果记录在
`artifacts/reports/p05_historical_remediation_2016_20260814.md`。
