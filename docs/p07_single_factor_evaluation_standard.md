...
- 任一成功处理期覆盖率低于冻结阈值；
- 显式选择预先声明的适用域时，任一期相对 P0.5 基础股票池的保留率低于冻结阈值；
...
`p07_single_factor_evaluation_v6` 把 P0.5 基础可研究资格与预先声明的因子适用域分开。
默认仍使用 `research_eligible` 作分母，不改变旧口径。只有因子输入在读取未来
收益前已冻结 `evaluation_eligible`、适用原因和研究域 SHA-256 时，才可显式选择
`eligibility_column=evaluation_eligible`。此时同时对适用域内因子覆盖率和相对 P0.5
基础股票池的保留率做独立硬门禁，默认均为 80%。这不允许根据 IC 或收益事后删股。
